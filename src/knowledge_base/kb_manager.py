...
class KBManager:
    ...
    def _create_kb_sync(self, kb_name: str, model: str):
        ...
        try:
            ...
            # Use Google Gemini - FREE with generous limits!
            # Model: text-embedding-004 (768 dimensions, excellent quality)
            # Note: Provider is "google" not "gemini" for embeddings in KB
            query = f"""
    ...
    def insert_papers_into_kb(self, kb_name: str = "academic_kb", db_name: str = "duckdb_papers") -> bool:
        ...
        try:
            ...
            print(f"   (This may take a few minutes for 49 papers)\n")
            
            # Insert papers into KB
            # MindsDB will automatically create embeddings for the content columns
            query = f"""
            INSERT INTO {kb_name}
